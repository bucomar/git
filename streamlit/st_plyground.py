...
st.title()
st.markdown()
    are used for headings and markdowns respectively.
st.sidebar() 
    is used for displaying data on the sidebar.
st.dataframe()
    to display the data frame 
st.map()
    to display the map in just a single line code etc.
'''

st.title('Egy kis COVID grafikon')

st.markdown('# Megpróbálok streamlit-tel összehozni valamit.')
st.markdown('## A vidó alapján nem nehéz...')
st.markdown('### Majd meglátjuk.')
